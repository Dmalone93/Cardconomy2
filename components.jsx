...
function CardArt({ item, w = 120, radius = 10, showFoil = true }) {
  ...
  React.useEffect(() => {
    let alive = true;
    setRealUrl(null); setLoaded(false);
    if (window.CardImg) window.CardImg.get(item, (url) => { if (alive && url) setRealUrl(url); });
    return () => { alive = false; };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: 'rgba(255,255,255,0.82)', fontFamily: T.mono, fontSize: Math.max(6.5, w*0.055),
      }}>
      ...
      {g && (
        <div style={{
          ...
          color: '#fff', fontFamily: T.sans, fontWeight: 800, fontSize: Math.max(7, w*0.075),
        }}>
          {g.name[0]}
        </div>
      )}
