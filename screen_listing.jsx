...
const { T: TL, money: moneyL, Slab: SlabL, CardArt: CardArtL, GradeChip: GradeChipL,
  ...
  CurrencyInput: CurrencyInputL, Container: ContainerL } = window;
...
function ListingScreen({ app, params }) {
  ...
  const priceHistoryBlock = item.history ? (
  ...
  const sellerDetailBlock = (
    <div style={{ borderTop: '1px solid var(--line-2)', marginTop: 6, paddingTop: 12 }}>
      {!isLot && (
  ...
  const tradeOffersBlock = product && product.tradeOffers && product.tradeOffers.length > 0 ? (
    <div style={{ borderTop: '1px solid var(--line-2)', marginTop: 6, paddingTop: 12 }}>
  ...
  const printingsBlock = PRINTINGS_L && PRINTINGS_L[item.name] && PRINTINGS_L[item.name].length > 1 ? (
    <div style={{ borderTop: '1px solid var(--line-2)', marginTop: 6, paddingTop: 12 }}>
  ...
  const similarBlock = (
    <div style={{ borderTop: '1px solid var(--line-2)', marginTop: 6, padding: '12px 0 16px' }}>
  ...
  const buyBoxBlock = (
    <div>
      <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 28, color: 'var(--ink)', marginBottom: 4 }}>{moneyL(displayPrice)}</div>
      <div style={{ fontFamily: TL.sans, fontSize: 12, color: 'var(--faint)', marginBottom: 16 }}>
        {item.shipping === 0 ? 'Free shipping' : moneyL(item.shipping) + ' shipping'} \u00B7 {item.loc}
      </div>
      <button onClick={() => app.addToCart(item.id)} style={{ width: '100%', background: 'var(--ink)', color: '#fff', borderRadius: 12,
        padding: '14px 8px', fontFamily: TL.sans, fontWeight: 700, fontSize: 15, border: 'none', marginBottom: 10,
        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        {app.cart.includes(item.id) ? 'In cart' : 'Add to cart'}
      </button>
      {item.accepts_offers && (
        <button onClick={() => setSheet('offer')} style={{ width: '100%', background: 'transparent', color: 'var(--ink)', borderRadius: 12,
          padding: '13px 8px', fontFamily: TL.sans, fontWeight: 600, fontSize: 14, border: '1px solid var(--line)', marginBottom: 10 }}>Make offer</button>
      )}
      <button onClick={() => app.nav.push('add_card', { cardId: item.id })} style={{ width: '100%',
        padding: '10px 12px', borderRadius: 8, background: 'transparent', color: 'var(--ink)',
        fontFamily: TL.sans, fontWeight: 600, fontSize: 13, border: '1px solid var(--line)' }}>I own this</button>
    </div>
  );
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)', animation: 'ccPushIn 0.26s ease' }}>
      {app.isWide ? (
        ...
        <div ref={scrollRefL} className="noscroll" style={{ flex: 1, overflow: 'auto' }}>
          <ContainerL style={{ padding: 0 }}>
            {/* top bar: back + share + watch */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 16px 0' }}>
              <button onClick={() => app.nav.pop()} style={{ width: 34, height: 34, borderRadius: 999, background: 'var(--surface)',
                color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--line-2)', cursor: 'pointer' }}>{IconL.back({})}</button>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { window.shareCard ? window.shareCard(item) : null; app.toast('Shared'); }} style={{ width: 34, height: 34, borderRadius: 999, background: 'var(--surface)',
                  color: 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--line-2)' }}>{IconL.share({})}</button>
                <button onClick={() => app.toggleWatch(item.id)} style={{ width: 34, height: 34, borderRadius: 999, background: 'var(--surface)',
                  color: watched ? TL.down : 'var(--muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--line-2)' }}>{IconL.heart({width:18,height:18}, watched)}</button>
              </div>
            </div>
            {/* 3-column grid */}
            <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr 300px', gap: 32, padding: '24px 16px 40px', alignItems: 'start' }}>
              {/* Left — sticky image + badges */}
              ...
              <div style={{ position: 'sticky', top: 80, alignSelf: 'flex-start' }}>
                <div style={{ background: 'var(--surface)', borderRadius: 16, padding: '20px 16px', border: '1px solid var(--line-2)' }}>
                  {buyBoxBlock}
                </div>
              </div>
            </div>
